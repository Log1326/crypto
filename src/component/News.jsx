...
import {useGetCryptoNewsQuery} from "../servies/cryptoNewsApi";
...
const News = ({simlified}) => {
    const [newsCategory, setNewsCategory] = useState('Cryptocurrency')
    const {data: cryptoNews} = useGetCryptoNewsQuery({newsCategory, count: simlified ? 6 : 12})
    const {data} = useGetCryptosQuery(100)
    const value = cryptoNews?.value
    ...
    return (
        <Row gutter={[24, 24]}>
            ...
            {value.map((news, i) =>
                <Col key={news.id} xs={24} sv={24} md={12} lg={8} xl={8}>
